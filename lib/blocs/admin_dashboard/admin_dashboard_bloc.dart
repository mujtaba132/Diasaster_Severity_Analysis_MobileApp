...
class AdminDashboardBloc
  ...
  final DashboardDonationRepository donationRepository;
  AdminDashboardBloc(
    this.ngoRepository,
    this.postRepository,
    ...
    on<OnLoadTotalDonationEvent>(_onLoadTotalDonation);
  }

  Future<void> _onLoadNgoDashboard(
    OnLoadNgoDashboardEvent event,
  ...
  Future<void> _onLoadUserDashboard(
    OnLoadUsersDashboardEvent event,
    Emitter<AdminDashboardState> emit,
  ) async {
    emit(state.copyWith(error: '', dashboardStatus: DashboardStatus.loading));
  ...
  Future<void> _onLoadPostDashboard(
    OnLoadPostDashboardEvent event,
    Emitter<AdminDashboardState> emit,
  ) async {
    emit(state.copyWith(error: '', dashboardStatus: DashboardStatus.loading));
  ...
  Future<void> _onLoadTotalDonation(
    OnLoadTotalDonationEvent event,
    Emitter<AdminDashboardState> emit,
  ) async {
    emit(state.copyWith(error: '', dashboardStatus: DashboardStatus.loading));

    await emit.forEach(
      donationRepository.getAllDonationAmount(),
      onData: (totalDonation) {
        return state.copyWith(
